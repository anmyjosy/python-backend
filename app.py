...
def graph(x):
    try:
        ...
        results = model.fit(disp=False)  # Suppress fitting output to save memory
        b = results.predict(start=120, end=140)
        future_df['forecast'] = results.predict(start=120, end=140)
        return b
    except Exception as e:
        logging.error(f"Error in graph function for attribute {x}: {e}")
        return None

@app.route('/cpi', methods=['GET'])
def get_cpi_data():
    attribute = request.args.get('attribute')
    logging.debug(f"Received request for attribute: {attribute}")
    ...
    if attribute in df.columns:
        ...
        if df1 is not None:
            ...
            data = [{"Date": "", "forecast": "Error processing data for this attribute"}]
    else:
        logging.warning(f"Attribute {attribute} not found in dataset")
